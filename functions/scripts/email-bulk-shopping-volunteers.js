...
async function main() {
  const { argv } = yargs
    ...
    .boolean('dry-run');
  const allRoutes = await getRecordsWithFilter(BULK_DELIVERY_ROUTES_TABLE, _.pick(argv, 'deliveryDate'));
  ...
  const bulkOrderRecords = await getRecordsWithFilter(BULK_ORDER_TABLE, _.pick(argv, 'deliveryDate'));
